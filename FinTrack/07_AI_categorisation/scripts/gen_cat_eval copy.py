...
def print_confidence_statistics(keyword_confidence):
    ...
    if confidences:
        ...
        plt.tight_layout()
        plt.savefig("confidence_histogram.png", dpi=120)
        plt.close()
...
def main():
    # Accept up to 4 arguments: transactions_in, categories_in, uncategorized_out, inconsistent_out
    if len(sys.argv) >= 5:
        ...
        categories_path = "AI_Categorisation_cleaned.json"
        uncategorized_path = "uncategorized_transactions.json"
        inconsistent_path = "inconsistent_categorizations.json"
